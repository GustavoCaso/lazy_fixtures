...
describe LazyFixtures::FactoryGirl::AssociationManager do
  ...
  describe '#columns_info' do
    it 'will return hash with reflections information' do
      return_value = described_class.new(@user).columns_info
      expect(return_value).to eq({'posts' => {method: "posts", macro: :has_many, klass: 'Post'}})
    end

    it 'will return hash with reflections information' do
      return_value = described_class.new(@post).columns_info
  ...
  describe '#determine_association' do
    it 'will return text for belongs to association' do
      user_association_info = described_class.new(@post).columns_info['user']
      return_value = described_class.new(@post).determine_association(user_association_info, 'User', 'user')
    ...
    it 'will return text for belongs to association with right association' do
      user_association_info = described_class.new(@post).columns_info['user']
      return_value = described_class.new(@post).determine_association(user_association_info, 'User', 'writter')
    ...
    it 'will return text for belongs to association with right factory' do
      user_association_info = described_class.new(@post).columns_info['user']
      return_value = described_class.new(@post).determine_association(user_association_info, 'Writter', 'user')
      expect(return_value).to eq "    association :user, factory: :writter\n"
    end


    it 'will return text for has_many to association' do
      post_association_info = described_class.new(@user).columns_info['posts']
      return_value = described_class.new(@user).determine_association(post_association_info, 'Post', 'post')
